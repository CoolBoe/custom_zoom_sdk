...
Future<void> checkAndDownloadSDK(String location) async {
  ...
  if (!exists) {
    await downloadFile(
        Uri.parse('https://www.dropbox.com/s/0mj5paq22qppql0/MobileRTC?dl=1'),
        iosSDKFile);
  }

  var iosSimulateSDKFile = location +
      '/ios/MobileRTC.xcframework/ios-i386_x86_64-simulator/MobileRTC.framework/MobileRTC-dev';
  exists = await File(iosSimulateSDKFile).exists();

  if (!exists) {
    await downloadFile(
        Uri.parse('https://www.dropbox.com/s/in0k6up2vvvk1c7/MobileRTC?dl=1'),
        iosSimulateSDKFile);
  }
