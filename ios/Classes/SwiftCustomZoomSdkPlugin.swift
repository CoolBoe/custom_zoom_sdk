...
public class SwiftCustomZoomSdkPlugin: NSObject, FlutterPlugin {
  ...
  public class ZoomViewHandler:NSObject, FlutterPlatfornViewFactory{
    private weak var messager:(NSObjectProtocol & FlutterBinaryMessenger)?
...
public class ZoomView:NSObject, FlutterPlatformView, MobileRTCMeetingServiceDelegate, FlutterStreamHandler{
  ...
  init(_ frame: CGRect, viewId:Int64, messanger:(NSObjectProtocol & FlutterBinaryMessenger)?, args:Any?){
    self.frame = frame
    self.viewId= viewId
    self.channel = FlutterMethodChannel(name:"custom_zoom_sdk", binaryMessanger:messenger!)
    self.authenticationDelegate = AuthenticationDelegate
    self.statusEventChannel = FlutterEventChannel(name:"custom_zoom_event", binaryMessanger:messenger!)

    super.init()

    self.statusEventChannel.setStreamHandler(self)
    self.channel.setMethodHandler(self.onMethodCall)
