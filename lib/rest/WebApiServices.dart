...
class WebApiServices {
  ...
  Future<bool> forgetPassword(String email) async {
    try {
      ...
      var params = new FormData.fromMap({'email': email});
      var response = await dio.post(url,
          options: new Options(headers: headers), data: params);
      if (response.statusCode == HTTP_CODE_200 ||
          response.statusCode == HTTP_CODE_201) {
  ...
  Future<Details> socialLogin(
    ...
    try {
      ...
      var response = await dio.post(url,
          options: new Options(headers: headers),
          data: mode == "google" ? googleParams : facebookParams);
      if (response.statusCode == HTTP_CODE_200 ||
          response.statusCode == HTTP_CODE_201) {
        UserModel userModel = UserModel.fromJson(response.data);
        if (userModel.code == '1') {
  ...
  Future<List<CategoryModel>> getCategories() async {
    try {
      ...
      if (response.statusCode == HTTP_CODE_200 ||
        ...
        List<CategoryModel> model = List<CategoryModel>.from(
  ...
  Future<List<ProductModel>> getProductByRelatedIds(
  ...
  Future<CartModel> getUpdateToCart(String cartItemKey, int quantity) async {
    try {
      ...
      String url = WebRequestConstants.getWPBaseUrl +
          ...
          params;
      var response =
          await dio.post(url, options: new Options(headers: headers));
  ...
  Future<bool> getClearToCart() async {
    try {
      ...
      String url = WebRequestConstants.getWPBaseUrl +
          WebRequestConstants.getDomainUrl +
          WebRequestConstants.CART_CLEAR;
      var response = await dio.get(url, options: new Options(headers: headers));
  ...
  Future<int> getCartItemCount() async {
    try {
      ...
      String url = WebRequestConstants.getWPBaseUrl +
          WebRequestConstants.getDomainUrl +
          WebRequestConstants.CART_ITEM_COUNT;
      var response = await dio.get(url, options: new Options(headers: headers));
      if (response.statusCode == HTTP_CODE_200 ||
          response.statusCode == HTTP_CODE_201) {
  ...
  Future<List<Coupons>> getCoupons() async {
    try {
      ...
      String url = WebRequestConstants.getWPBaseUrl +
          WebRequestConstants.getDomainUrl +
          WebRequestConstants.COUPON;
      var response = await dio.get(url,
          options: new Options(
              headers: {HttpHeaders.authorizationHeader: BaseAuth}));
      if (response.statusCode == HTTP_CODE_200 ||
          response.statusCode == HTTP_CODE_201) {
        Iterable l = response.data;
        List<Coupons> Cupons =
            List<Coupons>.from(l.map((model) => Coupons.fromJson(model)));
        return Cupons;
      } else {
        return null;
      }
  ...
  Future<ReviewOrder> getReviewOrder() async {
    try {
      ...
      String url = WebRequestConstants.getWPBaseUrl +
          WebRequestConstants.getDomainUrl +
          WebRequestConstants.REVIEW_ORDER;
      var response = await dio.get(url,
          options: new Options(
              headers: {HttpHeaders.authorizationHeader: BaseAuth}));
  ...
  Future<CartModel> getApplyCoupons({String coupon_code}) async {
    try {
      ...
      String url =
          "${WebRequestConstants.getWPBaseUrl}${WebRequestConstants.getDomainUrl}${WebRequestConstants.CART_COUPON}?coupon_code=$coupon_code";
  ...
  Future<CartModel> getRemoveCoupons({String coupon_code}) async {
    try {
      ...
      String url =
          "${WebRequestConstants.getWPBaseUrl}${WebRequestConstants.getDomainUrl}${WebRequestConstants.REMOVE_COUPON}?coupon_code=$coupon_code";
  ...
  Future<bool> getUpdateOrder(
      {String order_id, String status, String transaction_id}) async {
    try {
      Directory tempDir = await getApplicationDocumentsDirectory();
      String tempPath = tempDir.path;
      CookieJar sj = new PersistCookieJar(dir: tempPath, persistSession: true);
      ...
      String url = WebRequestConstants.getWPBaseUrl +
          WebRequestConstants.getDomainUrl +
          WebRequestConstants.UPDATE_ORDER;
      var response = await dio.post(url,
          options: new Options(headers: headers), data: params);
    ...
    try {
      ...
      if (response.statusCode == HTTP_CODE_200 ||
        ...
        List<CityModel> model =
            List<CityModel>.from(l.map((model) => CityModel.fromJson(model)));
        return model;
      } else {
        return null;
      }
  ...
  Future<bool> updateBilling({String user_id, Billing shipping}) async {
    try {
      ...
      String url = WebRequestConstants.getWPBaseUrl +
          WebRequestConstants.getDomainUrl +
          WebRequestConstants.UPDATE_BILLING;
      printLog("UpdateBilling", url);
      printLog("UpdateBilling", params);
      var response = await dio.post(url,
          options:
              new Options(headers: {HttpHeaders.authorizationHeader: BaseAuth}),
          data: json.encode(params));
      printLog("UpdateBilling", response);
      if (response.statusCode == HTTP_CODE_200 ||
          response.statusCode == HTTP_CODE_201) {
  ...
  Future<List<ColorSizeModel>> getColorSizeList({String id}) async {
    try {
      ...
      if (response.statusCode == HTTP_CODE_200 ||
          response.statusCode == HTTP_CODE_201) {
        Iterable l = response.data;
        List<ColorSizeModel> model = List<ColorSizeModel>.from(
            l.map((model) => ColorSizeModel.fromJson(model)));
  ...
  Future<List<PaymentGateway>> getPaymentGateway() async {
    try {
      ...
      if (response.statusCode == HTTP_CODE_200 ||
          response.statusCode == HTTP_CODE_201) {
        Iterable l = response.data;
        List<PaymentGateway> model = List<PaymentGateway>.from(
            l.map((model) => PaymentGateway.fromJson(model)));
        return model;
      } else {
        return null;
      }
  ...
  Future<AppSetting> getAppSetting() async {
    try {
      ...
      String url = WebRequestConstants.getWPBaseUrl +
          WebRequestConstants.getDomainUrl +
          WebRequestConstants.APP_SETTING;
